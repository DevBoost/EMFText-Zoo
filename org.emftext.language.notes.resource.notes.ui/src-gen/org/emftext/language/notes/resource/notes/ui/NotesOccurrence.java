 ...
 */
public class NotesOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.notes.resource.notes.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.notes.resource.notes.ui.occurences.declaration";
	...
	private org.emftext.language.notes.resource.notes.ui.NotesTokenScanner tokenScanner;
	...
	private org.emftext.language.notes.resource.notes.INotesTextResource textResource;
	 ...
	 */
	public NotesOccurrence(org.emftext.language.notes.resource.notes.INotesTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.notes.resource.notes.ui.NotesTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.notes.resource.notes.mopp.NotesMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.notes.resource.notes.ui.NotesBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.notes.resource.notes.ui.NotesPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.notes.resource.notes.ui.NotesPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.notes.resource.notes.ui.NotesPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.notes.resource.notes.ui.NotesPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.notes.resource.notes.ui.NotesOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.notes.resource.notes.ui.NotesOccurrence.DECLARATION_ANNOTATION_ID);
	}
