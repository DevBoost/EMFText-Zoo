 ...
 * </ul>
 */
public class NotesBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new org.emftext.language.notes.resource.notes.mopp.NotesMetaInformation().getSyntaxName();
	
	private org.emftext.language.notes.resource.notes.ui.NotesBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public NotesBracketPreferencePage() {
		super();
		
		org.emftext.language.notes.resource.notes.INotesMetaInformation metaInformation = new org.emftext.language.notes.resource.notes.mopp.NotesMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(org.emftext.language.notes.resource.notes.ui.NotesUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new org.emftext.language.notes.resource.notes.ui.NotesBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					org.emftext.language.notes.resource.notes.INotesBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					org.emftext.language.notes.resource.notes.INotesBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + org.emftext.language.notes.resource.notes.ui.NotesPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof org.emftext.language.notes.resource.notes.ui.NotesEditor) {
			((org.emftext.language.notes.resource.notes.ui.NotesEditor) editor).invalidateTextRepresentation();
		}
	}
