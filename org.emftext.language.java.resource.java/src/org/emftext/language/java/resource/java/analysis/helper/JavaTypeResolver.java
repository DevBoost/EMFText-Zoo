...
public class JavaTypeResolver<T extends EObject, R extends Classifier> extends 
	JavaReferenceResolver<T, R> {
	
	@Override	
	protected java.lang.String doDeResolve(R classifier, T container, org.eclipse.emf.ecore.EReference reference) {
		...
		if (!(container.eContainer() instanceof NamespaceClassifierReference)) {
			...
			URI resourceURI = classifier.eResource().getURI();
					...
					if (fullID.contains("$")) {
						...
						for(Import imp : cu.getImports()) {
							if(imp instanceof ClassifierImport) {
								...
								if (classifier.equals(classifierImport.getClassifier())) {
							...
							if(imp instanceof PackageImport) {
								...
								if (staticImport.getStaticMembers().contains(classifier)) {
									//the element is imported -> simple name
									fullID = classifier.getName();
								}
							}
	...
	@Override	
	protected void doResolve(java.lang.String identifier, T container, org.eclipse.emf.ecore.EReference reference, int position, boolean resolveFuzzy, IReferenceResolveResult<R> result) {
		...
		if  (!result.wasResolved()){
			...
			NamespaceAwareElement nsElement = null;
			if (container instanceof NamespaceAwareElement) {
				nsElement = (NamespaceAwareElement) container;
			}
			else if (container.eContainer() instanceof NamespaceAwareElement) {
				nsElement = (NamespaceAwareElement) container.eContainer();
			}
			if (nsElement == null) {
				return;
			}
			...
			//global
			String containerName = JavaClasspath.INSTANCE.getContainerNameFromNamespace(nsElement, "");
			if (containerName == null) {
				return;
			}
			//append possible type-argumented container classes
			if(nsElement instanceof NamespaceClassifierReference) {
				myPos = ((NamespaceClassifierReference)nsElement).getClassifierReferences().indexOf(container);
				for(int i = 0; i < myPos; i++) {
					Classifier prevClassifier = ((NamespaceClassifierReference)nsElement).getClassifierReferences().get(i).getTarget();
			...
			if (target == null) {
				...
				if (myPos > 0) {
					...
					directContainer = (ConcreteClassifier) ((NamespaceClassifierReference)nsElement).getClassifierReferences().get(myPos - 1).getTarget();
				}
				else if (!nsElement.getNamespaces().isEmpty()) {
					...
					if (!myResource.getContents().isEmpty()) {
						JavaRoot rootElement = (JavaRoot) myResource.getContents().get(0);
						collectClassifiersInScope(
								findContainingClassifier(container), rootElement, rootContainerCandidates);	
					}
					
					for(String nextInNS : nsElement.getNamespaces()) {
			...
			if (target != null) {
				result.addMapping(identifier, (R) target);
			}
		}			
	...
	protected void collectClassifiersInScope(
			...
			NamespaceAwareElement cuOrPackage,
			EList<ConcreteClassifier> rootContainerCandidates) {

		
		CompilationUnit cu = null;
		if(cuOrPackage instanceof CompilationUnit) {
			cu = (CompilationUnit) cuOrPackage;
		}
		...
		String packageName = JavaUniquePathConstructor.packageName(cuOrPackage);
