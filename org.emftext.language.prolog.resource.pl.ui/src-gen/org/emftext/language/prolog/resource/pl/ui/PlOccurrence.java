...
public class PlOccurrence {
	...
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.prolog.resource.pl.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.prolog.resource.pl.ui.occurences.declaration";
	...
	private org.emftext.language.prolog.resource.pl.ui.PlTokenScanner tokenScanner;
	 ...
	 */
	public PlOccurrence(org.emftext.language.prolog.resource.pl.IPlTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.prolog.resource.pl.ui.PlTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.prolog.resource.pl.mopp.PlMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.prolog.resource.pl.ui.PlBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.prolog.resource.pl.ui.PlPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.prolog.resource.pl.ui.PlPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.prolog.resource.pl.ui.PlPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.prolog.resource.pl.ui.PlPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.prolog.resource.pl.ui.PlOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.prolog.resource.pl.ui.PlOccurrence.DECLARATION_ANNOTATION_ID);
	}
