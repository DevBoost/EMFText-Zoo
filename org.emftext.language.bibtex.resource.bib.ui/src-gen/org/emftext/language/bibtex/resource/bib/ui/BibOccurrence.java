 ...
 */
public class BibOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.bibtex.resource.bib.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.bibtex.resource.bib.ui.occurences.declaration";
	...
	private org.emftext.language.bibtex.resource.bib.ui.BibTokenScanner tokenScanner;
	...
	private org.emftext.language.bibtex.resource.bib.IBibTextResource textResource;
	 ...
	 */
	public BibOccurrence(org.emftext.language.bibtex.resource.bib.IBibTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.bibtex.resource.bib.ui.BibTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.bibtex.resource.bib.mopp.BibMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.bibtex.resource.bib.ui.BibBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.bibtex.resource.bib.ui.BibPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.bibtex.resource.bib.ui.BibPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.bibtex.resource.bib.ui.BibPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.bibtex.resource.bib.ui.BibPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.bibtex.resource.bib.ui.BibOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.bibtex.resource.bib.ui.BibOccurrence.DECLARATION_ANNOTATION_ID);
	}
