...
public class AbnfOccurrence {
	...
	private org.emftext.language.abnf.resource.abnf.ui.AbnfTokenScanner tokenScanner;
	...
	private org.emftext.language.abnf.resource.abnf.IAbnfTextResource textResource;
	 ...
	 */
	public AbnfOccurrence(org.emftext.language.abnf.resource.abnf.IAbnfTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.abnf.resource.abnf.ui.AbnfTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.abnf.resource.abnf.mopp.AbnfMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.abnf.resource.abnf.ui.AbnfBracketSet bracketSet) {
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.emftext.language.abnf.resource.abnf.ui.AbnfPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.emftext.language.abnf.resource.abnf.ui.AbnfPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
