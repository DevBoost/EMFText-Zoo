 ...
 */
public class NotesMinimalModelHelper {
	
	private final static org.emftext.language.notes.resource.notes.util.NotesEClassUtil eClassUtil = new org.emftext.language.notes.resource.notes.util.NotesEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + org.emftext.language.notes.resource.notes.util.NotesStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = org.emftext.language.notes.resource.notes.util.NotesListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + org.emftext.language.notes.resource.notes.util.NotesStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = org.emftext.language.notes.resource.notes.util.NotesListUtil.castListUnchecked(value);
