...
public class NotesDefaultNameProvider implements org.emftext.language.notes.resource.notes.INotesNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.notes.resource.notes.util.NotesEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
