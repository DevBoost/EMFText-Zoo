...
public class Uml_classOccurrence {
	...
	private org.emftext.language.uml_class.resource.uml_class.ui.Uml_classTokenScanner tokenScanner;
	...
	///
	public Uml_classOccurrence(org.emftext.language.uml_class.resource.uml_class.IUml_classTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.uml_class.resource.uml_class.ui.Uml_classTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.uml_class.resource.uml_class.mopp.Uml_classMetaInformation().getTokenNames();
	...
	public void handleOccurrenceHighlighting(org.emftext.language.uml_class.resource.uml_class.ui.Uml_classBracketSet bracketSet) {
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		int caretOffset = textWidget.getCaretOffset();
		caretOffset = projectionViewer.widgetOffset2ModelOffset(caretOffset);
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addPosition(document, org.emftext.language.uml_class.resource.uml_class.ui.Uml_classPositionCategory.DEFINTION.toString());
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if ((isNull && elementsAtDefinition.contains(occEO)) || !isNull && definitionElement.equals(occEO)) {
						addPosition(document, org.emftext.language.uml_class.resource.uml_class.ui.Uml_classPositionCategory.PROXY.toString());
					}
				}
			}
			token = tokenScanner.nextToken();
