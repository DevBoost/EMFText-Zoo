...
public class PlDefaultNameProvider implements org.emftext.language.prolog.resource.pl.IPlNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = org.emftext.language.prolog.resource.pl.util.PlEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
